...
class MyCephCollector(diamond.collector.Collector):

    def get_default_config_help(self):
        config_help = super(MyCephCollector, self).get_default_config_help()
        config_help.update({
    ...
    def get_default_config(self):
        ...
        """
        config = super(MyCephCollector, self).get_default_config()
        config.update({
            'socket_path': '/home/cai/ceph/build/out',
            'socket_prefix': '',
            'socket_ext': 'asok',
    ...
    def _get_stats_from_socket(self, name):
        ...
        """
        try:
	    command = 'cd /home/cai/ceph/build/ &&'+self.config['ceph_binary']+' --admin-daemon '+name+' perf'+' dump'+" && cd /"
	    self.log.info(command)
	    self.log.exception(command)
